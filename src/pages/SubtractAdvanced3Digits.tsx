...
export default function SubtractAdvanced3Digits() {
  ...
  const generateExercise = (level: number, existingQuestions: Set<string> = new Set()): Exercise => {
    ...
    switch (level) {
      case 1: // Easy: 200-399 minus 100-199
        num1 = Math.floor(Math.random() * 200) + 200;
        num2 = Math.floor(Math.random() * 100) + 100;
        break;
      case 2: // Medium-easy: 300-599 minus 100-299
        num1 = Math.floor(Math.random() * 300) + 300;
        num2 = Math.floor(Math.random() * 200) + 100;
        break;
      case 3: // Medium: 400-699 minus 200-399
        num1 = Math.floor(Math.random() * 300) + 400;
        num2 = Math.floor(Math.random() * 200) + 200;
        break;
      case 4: // Hard: 500-799 minus 200-499
        num1 = Math.floor(Math.random() * 300) + 500;
        num2 = Math.floor(Math.random() * 300) + 200;
        break;
      case 5: // Very hard: 600-999 minus 300-599
        num1 = Math.floor(Math.random() * 400) + 600;
        num2 = Math.floor(Math.random() * 300) + 300;
        break;
      default:
        num1 = Math.floor(Math.random() * 300) + 300;
        num2 = Math.floor(Math.random() * 200) + 100;
  ...
  const handleSubmit = () => {
    ...
    if (isAnswerCorrect) {
      ...
      toast({
        title: streak >= 2 ? `🎉 ${streak + 1} ${t("subtractAdvanced3Digits.inARow")}` : `✅ ${t("subtractAdvanced3Digits.correct")}`,
        status: "success",
        duration: 1500,
        isClosable: true,
        position: "top",
      ...
      toast({
        title: `❌ ${t("subtractAdvanced3Digits.notQuite")}`,
        description: `${t("subtractAdvanced3Digits.answerIs")} ${currentExercise.answer}`,
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "top",
    ...
    if (maxExercises && totalQuestions + 1 >= maxExercises) {
      ...
      if (currentUser) {
        ...
        const testResult = {
          testType: "subtract-advanced-3digits",
          score: finalScore,
        ...
        <VStack spacing={8}>
          <HStack justify="center" spacing={4}>
            <Heading textAlign="center" color="pink.600" size="lg">
              {t("subtractAdvanced3Digits.title")}
            </Heading>
          ...
          {tempCount && (
            <>
              ...
              <Button
                ...
                variant="ghost"
              >
                ← {t("subtractAdvanced3Digits.back")}
              </Button>
            </>
          )}
        </VStack>
        ...
        <VStack spacing={8}>
          ...
          >
            <Heading size="2xl" textAlign="center">
              {percentage === 100 ? t("subtractAdvanced3Digits.perfectScore") : percentage >= 80 ? t("subtractAdvanced3Digits.greatJob") : percentage >= 60 ? t("subtractAdvanced3Digits.wellDone") : t("subtractAdvanced3Digits.keepPracticing")}
            </Heading>
          </MotionBox>
          ...
          >
            <VStack spacing={6}>
              <Heading size="lg" color="gray.700">
                {t("subtractAdvanced3Digits.testComplete")}
              </Heading>

              <HStack spacing={8} fontSize="3xl" fontWeight="bold">
                <VStack>
                  <Text color="gray.500" fontSize="lg">{t("subtractAdvanced3Digits.score")}</Text>
                  <Text color="pink.600">{score}/{totalQuestions}</Text>
                </VStack>
                <VStack>
                  <Text color="gray.500" fontSize="lg">{t("subtractAdvanced3Digits.accuracy")}</Text>
                  <Text color="pink.600">{percentage}%</Text>
                </VStack>
                <VStack>
                  <Text color="gray.500" fontSize="lg">{t("subtractAdvanced3Digits.time")}</Text>
          ...
          <HStack spacing={4} width="100%">
            <Button
              ...
              flex={1}
            >
              {t("subtractAdvanced3Digits.startNew")}
            </Button>
            <Button
              ...
              flex={1}
            >
              {t("subtractAdvanced3Digits.backToTests")}
            </Button>
          </HStack>

          <Box width="100%">
            <Heading size="md" mb={4} color="gray.600">
              {t("subtractAdvanced3Digits.reviewAnswers")}
            </Heading>
      ...
      <VStack spacing={6} align="stretch">
        <Flex justify="flex-end" align="center" wrap="wrap" gap={2}>
          <HStack spacing={4}>
            ...
            <Badge colorScheme="purple" fontSize="lg" px={4} py={2} borderRadius="full">
              🔥 {t("subtractAdvanced3Digits.streak")}: {streak}
            </Badge>
          </HStack>
        </Flex>
        ...
        <Box>
          <Flex justify="space-between" mb={2}>
            <Text fontWeight="bold">
              {t("subtractAdvanced3Digits.progress")}: {totalQuestions}/{maxExercises}
            </Text>
            <Text fontWeight="bold">
              {t("subtractAdvanced3Digits.score")}: {score}/{totalQuestions} ({totalQuestions > 0 ? Math.round((score / totalQuestions) * 100) : 0}%)
            </Text>
          </Flex>
          <Progress
          ...
          >
            <VStack spacing={8}>
              ...
              <Button
                ...
                transition="all 0.2s"
              >
                {t("subtractAdvanced3Digits.checkAnswer")}
              </Button>
            </VStack>
          </Box>
        </MotionBox>
        ...
        >
          <Text textAlign="center" fontSize="xl" color="gray.500">
            {streak >= 5
              ? t("subtractAdvanced3Digits.onFire")
              : streak >= 3
                ? t("subtractAdvanced3Digits.amazing")
                : t("subtractAdvanced3Digits.typeAnswer")}
          </Text>
        </MotionBox>
        ...
        {completedExercises.length > 0 && (
          <Box mt={8}>
            <Heading size="md" mb={4} color="gray.600">
              {t("subtractAdvanced3Digits.completedExercises")}
            </Heading>
              ...
              {completedExercises.map((exercise, index) => (
                ...
                >
                  <Box
                    ...
                    title={t("subtractAdvanced3Digits.clickToRetry")}
                  >
                    <Flex justify="space-between" align="center">
                      <HStack spacing={4} fontSize="2xl" fontWeight="bold">
                        <Text color={exercise.isCorrect ? "green.600" : "red.600"}>
