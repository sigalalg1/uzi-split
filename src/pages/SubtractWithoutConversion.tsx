...
export default function SubtractWithoutConversion() {
  ...
  const handleSubmit = () => {
    ...
    if (isAnswerCorrect) {
      ...
      toast({
        title: streak >= 2 ? `🎉 ${streak + 1} ${t("subtractWithoutConversion.inARow")}` : `✅ ${t("subtractWithoutConversion.correct")}`,
        status: "success",
        duration: 1500,
        isClosable: true,
        position: "top",
      ...
      toast({
        title: `❌ ${t("subtractWithoutConversion.notQuite")}`,
        description: `${t("subtractWithoutConversion.answerIs")} ${currentExercise.answer}`,
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "top",
    ...
    if (maxExercises && totalQuestions + 1 >= maxExercises) {
      ...
      if (currentUser) {
        ...
        const testResult = {
          testType: "subtract-without-conversion",
          score: finalScore,
        ...
        <VStack spacing={8}>
          <HStack justify="center" spacing={4}>
            <Heading textAlign="center" color="pink.600" size="lg">
              {t("subtractWithoutConversion.title")}
            </Heading>
          ...
          {tempCount && (
            <>
              ...
              <Button
                ...
                variant="ghost"
              >
                ← {t("subtractWithoutConversion.back")}
              </Button>
            </>
          )}
        </VStack>
        ...
        <VStack spacing={8}>
          ...
          >
            <Heading size="2xl" textAlign="center">
              {percentage === 100 ? t("subtractWithoutConversion.perfectScore") : percentage >= 80 ? t("subtractWithoutConversion.greatJob") : percentage >= 60 ? t("subtractWithoutConversion.wellDone") : t("subtractWithoutConversion.keepPracticing")}
            </Heading>
          </MotionBox>
          ...
          >
            <VStack spacing={6}>
              <Heading size="lg" color="gray.700">
                {t("subtractWithoutConversion.testComplete")}
              </Heading>

              <HStack spacing={8} fontSize="3xl" fontWeight="bold">
                <VStack>
                  <Text color="gray.500" fontSize="lg">{t("subtractWithoutConversion.score")}</Text>
                  <Text color="pink.600">{score}/{totalQuestions}</Text>
                </VStack>
                <VStack>
                  <Text color="gray.500" fontSize="lg">{t("subtractWithoutConversion.accuracy")}</Text>
                  <Text color="pink.600">{percentage}%</Text>
                </VStack>
                <VStack>
                  <Text color="gray.500" fontSize="lg">{t("subtractWithoutConversion.time")}</Text>
          ...
          <HStack spacing={4} width="100%">
            <Button
              ...
              flex={1}
            >
              {t("subtractWithoutConversion.startNew")}
            </Button>
            <Button
              ...
              flex={1}
            >
              {t("subtractWithoutConversion.backToTests")}
            </Button>
          </HStack>

          <Box width="100%">
            <Heading size="md" mb={4} color="gray.600">
              {t("subtractWithoutConversion.reviewAnswers")}
            </Heading>
      ...
      <VStack spacing={6} align="stretch">
        <Flex justify="flex-end" align="center" wrap="wrap" gap={2}>
          <HStack spacing={4}>
            ...
            <Badge colorScheme="purple" fontSize="lg" px={4} py={2} borderRadius="full">
              🔥 {t("subtractWithoutConversion.streak")}: {streak}
            </Badge>
          </HStack>
        </Flex>
        ...
        <Box>
          <Flex justify="space-between" mb={2}>
            <Text fontWeight="bold">
              {t("subtractWithoutConversion.progress")}: {totalQuestions}/{maxExercises}
            </Text>
            <Text fontWeight="bold">
              {t("subtractWithoutConversion.score")}: {score}/{totalQuestions} ({totalQuestions > 0 ? Math.round((score / totalQuestions) * 100) : 0}%)
            </Text>
          </Flex>
          <Progress
          ...
          >
            <VStack spacing={8}>
              ...
              <Button
                ...
                transition="all 0.2s"
              >
                {t("subtractWithoutConversion.checkAnswer")}
              </Button>
            </VStack>
          </Box>
        </MotionBox>
        ...
        >
          <Text textAlign="center" fontSize="xl" color="gray.500">
            {streak >= 5
              ? t("subtractWithoutConversion.onFire")
              : streak >= 3
                ? t("subtractWithoutConversion.amazing")
                : t("subtractWithoutConversion.typeAnswer")}
          </Text>
        </MotionBox>
        ...
        {completedExercises.length > 0 && (
          <Box mt={8}>
            <Heading size="md" mb={4} color="gray.600">
              {t("subtractWithoutConversion.completedExercises")}
            </Heading>
              ...
              {completedExercises.map((exercise, index) => (
                ...
                >
                  <Box
                    ...
                    title={t("subtractWithoutConversion.clickToRetry")}
                  >
                    <Flex justify="space-between" align="center">
                      <HStack spacing={4} fontSize="2xl" fontWeight="bold">
                        <Text color={exercise.isCorrect ? "green.600" : "red.600"}>
